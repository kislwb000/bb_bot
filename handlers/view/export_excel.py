...
async def handle_export_excel_start(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=event.title, callback_data=f"export_event:{event.id}")]
        for event in events
    ] + back_button("view_database"))

    await callback.message.edit_text("📁 Выбери субботник для экспорта:", reply_markup=kb) # type: ignore
...
async def handle_export_event(callback: CallbackQuery):
    ...
    async with async_session() as session:
        stmt = select(Team).where(Team.event_id == event_id).options(selectinload(Team.scores))
    ...
    category_titles = {key: title for key, (title, _) in CATEGORIES.items()}
    ...
    # 🔠 Заголовки
    headers = ["Команда"] + [category_titles[key] for key in category_keys] + ["Всего баллов"]
    ...
    # 🧮 Подсчёт мешков по категориям
    category_totals = {key: 0 for key in category_keys}
    for team in teams:
        for score in team.scores:
            if score.category in category_totals:
                category_totals[score.category] += score.points

    # 📦 Добавим итоги по категориям
    ...
    # 📤 Отправка
    await callback.message.answer_document(FSInputFile(path=filename, filename=filename), reply_markup=kb) # type: ignore
    app_logger.info(f"📤 Экспорт Excel: Субботник '{event.title}' (ID {event.id}) — админ {callback.from_user.id} | @{callback.from_user.username or '-'}") # type: ignore
    os.remove(filename)
